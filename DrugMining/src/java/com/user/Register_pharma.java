...
public class Register_pharma extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(rs.getInt(1)==0)
            {
                connect.executeUpdate("Insert into users (id,fullname,username,password,address,email,contact_no,user_type) values(null,'"+name+"','"+username+"','"+password+"','"+address+"','"+email+"','"+contact+"','pharma')");
                session.setAttribute("flash_message", "Registered successfully !!!");
                session.setAttribute("flash_type", "success");
                response.sendRedirect("view_pharmacist.jsp");
            }
            else
            {
                session.setAttribute("flash_message", "User ID Is Already Alloted");
                session.setAttribute("flash_type", "Fail");
                response.sendRedirect("register_pharmacist.jsp");
            }
        } catch(Exception e) {            
